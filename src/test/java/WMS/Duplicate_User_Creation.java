...
public class Duplicate_User_Creation {
	...
	By Add_New_Record = By.xpath("//a[normalize-space()='Add new record']");
	...
	By Submit = By.xpath("//a[normalize-space()='Update']");
	...
	String Actual_Err_Mes;
	String Expeced_Err_Mes = "Invalid / Duplicate Username";
	int j = 0;
	...
	public void Setup_Driver() throws IOException {
		...
		inputStream = new FileInputStream(
				new File("C:\\Users\\Abdelrahman\\OneDrive\\Desktop\\cc\\cc\\resources\\Duplicate.xlsx"));
	...
	@Test(priority = 3)
	public void Open_User_Master_Page() throws InterruptedException {
	...
	public void Add_user_credentials() {
		...
		userid.sendKeys(sheet.getRow(j).getCell(0).toString());
		username = driver.findElement(UserName);
		username.sendKeys(sheet.getRow(j).getCell(1).toString());
	...
	public void Choose_user_store() throws InterruptedException {
		...
		wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//li[normalize-space()='1001']")));
	...
	public void submit() throws Exception {
		...
		Actual_Err_Mes = driver.switchTo().alert().getText();
		assertEquals(Actual_Err_Mes, Expeced_Err_Mes);
		driver.switchTo().alert().accept();

		int UsersNumber = sheet.getLastRowNum() + 1;

		for (int i = j + 1; i < UsersNumber; i++) {
			j++;
			this.Create_new_record();
			this.Add_user_credentials();
			this.Choose_User_Type();
			this.Choose_user_store();
			this.Check_boxes();
			submit = driver.findElement(Submit);
			submit.click();
			wait.until(ExpectedConditions.alertIsPresent());
